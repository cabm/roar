...
class MasonJsonTest < MiniTest::Spec
  ...
  describe "links" do
    describe "parsing" do
      ...
      it "parses link and link array" do
        ...
        obj.controls.must_equal "next"=>link("rel" => "next", "href" => "http://next"), "self"=>[link("rel" => "self", "href" => "http://en.hit", "lang" => "en"), link("rel" => "self", "href" => "http://de.hit", "lang" => "de")]
      end

      it "parses empty link array" do
        subject.from_json("{\"@controls\":{\"self\":[]}}").controls[:self].must_equal nil
      end
  ...
  describe "empty link array" do
    representer!([Roar::JSON::Mason2]) do
      controls(:self) { [] }
    end

    it "gets render" do
      Object.new.extend(representer).to_json.must_equal "{\"@controls\":{\"self\":[]}}"
    end
  end
...
class HalCurieTest < MiniTest::Spec
  ...
  it { Object.new.extend(rpr).to_hash.must_equal({"@controls"=>{"doc:self"=>{"href"=>"/"}, "curies"=>[{"name"=>:doc, "href"=>"//docs/{rel}", "templated"=>true}]}}) }
end
